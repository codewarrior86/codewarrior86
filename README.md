<!-- Banner / Header -->
<h1 align="center">Hi 👋, I'm Muhammad Arjun</h1>
<h3 align="center">Frontend Web Developer | React & Next.js Enthusiast</h3>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=muhammadarjun&label=Profile%20Views&color=0e75b6&style=flat" alt="profile views" />
</p>

---

## 🚀 About Me  
I'm a **Frontend Web Developer** who loves crafting interactive, fast, and modern web experiences.  
Specializing in building beautiful UI with clean code, smooth animations, and high-performance architecture.

- 🔭 Currently working with **React, Next.js & TypeScript**
- ✨ Passionate about **UI animation (GSAP, Framer Motion)**
- 🎨 Love building aesthetic UI with **Tailwind CSS & Material UI**
- ⚡ Focus on performance, accessibility, and modern frontend patterns
- 📚 Always learning & exploring new web tech

---

## 🛠 Tech Stack

### 🚀 Frontend
<p>
  <img src="https://skillicons.dev/icons?i=react,nextjs,typescript,js,html,css,tailwind,bootstrap,materialui" />
</p>

### 🎨 Animation & UI Enhancements
<p>
  <img src="https://skillicons.dev/icons?i=framer" />
  <img src="https://img.shields.io/badge/GSAP-88CE02?style=for-the-badge&logo=greensock&logoColor=white" />
</p>

### 🔧 Tools
<p>
  <img src="https://skillicons.dev/icons?i=git,github,vscode,figma" />
</p>

---

## ✨ Featured Projects

### 🛒 **E-Commerce Fullstack (Next.js 14)**
- Next.js App Router + Zustand
- Tailwind CSS + Shadcn UI
- API integration + Pagination + Cart System  
➡️ *Coming soon to GitHub!*

### 📚 Personal Portfolio
- Beautiful animations using Framer Motion
- Modern responsive layout
- Built with Next.js + TypeScript  
🔗 https://portfolio-arjun.web.app/

---

## 📈 GitHub Stats
<p align="center">
  <img height="170" src="https://github-readme-stats.vercel.app/api?username=muhammadarjun&show_icons=true&theme=tokyonight" />
  <img height="170" src="https://github-readme-stats.vercel.app/api/top-langs/?username=muhammadarjun&layout=compact&theme=tokyonight" />
</p>

---

## 🔗 Connect With Me
<p>
  <a href="https://linkedin.com/in/yourlinkedin"><img src="https://skillicons.dev/icons?i=linkedin" height="40" /></a>
  <a href="https://instagram.com/yourinstagram"><img src="https://skillicons.dev/icons?i=instagram" height="40" /></a>
  <a href="mailto:youremail@gmail.com"><img src="https://skillicons.dev/icons?i=gmail" height="40" /></a>
</p>

---

## ⚡ Fun Quote  
> "Great frontend isn't just about code — it's about crafting experiences."




<!--
## Hi there 👋
-->

<!--
**codewarrior86/codewarrior86** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
